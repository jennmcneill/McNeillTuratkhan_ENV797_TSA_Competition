---
title: "Forecasting Competition Final Report - McNeill/Turatkhan"
subtitle: "[GitHub Repository Link](https://github.com/jennmcneill/McNeillTuratkhan_ENV797_TSA_Competition)"
author: "Jenn McNeill and Zhanylai Turatkhan kyzy"
date: "04/01/2024"
output: pdf_document
...
```{r create time series objects, include=FALSE}
...
```{r decompose time series objects, include=FALSE}
...
```{r TBATS}
TBATS_fit <- tbats(ts_load_daily_training)
...
```{r TBATS scores and retraining}
TBATS_scores <- accuracy(TBATS_for$mean, ts_load_daily_testing)
print(TBATS_scores)

#retraining for the whole dataset to predict july 2011
TBATS_kaggle_fit <- tbats(ts_load_daily)
TBATS_kaggle_for <- forecast(TBATS_kaggle_fit, h=31)

TBATS_forecast_submission <- TBATS_kaggle_for$mean

# the best score as of now in Kaggle
getwd()
submission_template <- read.csv(file="./submission_template.csv", header=TRUE)
submission_template$date <- as.Date(submission_template$date, format = "%m/%d/%y")
submission_template$load <- TBATS_forecast_submission
write.table(submission_template, "submission.csv", sep = ",", row.names = FALSE, quote = FALSE)
```
